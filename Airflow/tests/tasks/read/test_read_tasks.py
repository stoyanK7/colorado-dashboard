...
class TestReadTasks(unittest.TestCase):
    ...
    def test_get_files_to_data_frames_with_empty_last_seen(self, mock_file_reader: MagicMock, mock_os: MagicMock, mock_var: MagicMock):
        ...
        result_data_frames = ReadTasks._get_files_to_data_frames(directory_variable_key, files_to_read, last_seen_file, last_seen_row)
        ...
        mock_os.assert_called_once_with("AIRFLOW_HOME")
        mock_var.assert_has_calls([unittest.mock.call(directory_variable_key), unittest.mock.call("image_col_name_ullid")])

    @patch("airflow.models.variable.Variable.get")
    @patch("os.getenv")
    @patch("tasks.read.file_reader.FileReader.read_pandas_csv_file")
    def test_get_files_to_data_frames_with_last_seen(self, mock_file_reader: MagicMock, mock_os: MagicMock, mock_var: MagicMock):
        df1 = pd.DataFrame(data={"ullid":[0, 1, 2, 3], "values": ["value1", "value2", "value3", "value4"]})
        df2 = pd.DataFrame(data={"ullid": [4, 5, 6, 7], "values": ["value5", "value6", "value7", "value8"]})
        mock_file_reader.side_effect = [df1, df2]
        mock_os.return_value = "Some/Os/Path/"
        ...
        pd.testing.assert_frame_equal(result_data_frames, expected)
        mock_file_reader.assert_has_calls([unittest.mock.call("Some/Os/Path/To/The/Files/file1", ";"), unittest.mock.call("Some/Os/Path/To/The/Files/file2", ";")])
        mock_os.assert_called_once_with("AIRFLOW_HOME")
        mock_var.assert_has_calls([unittest.mock.call(directory_variable_key), unittest.mock.call("image_col_name_ullid")])
    ...
    def test_makeXcom(self):
        ...
        # Assert
        ti.xcom_push.assert_has_calls([unittest.mock.call("last_seen_file", "someFile"), unittest.mock.call("last_seen_row", "someRow")])
    ...
    def test_read_image_full(self, mock_get_last_seen: MagicMock, mock_file_names: MagicMock, mock_files_to_df: MagicMock, mock_change_col_names: MagicMock, mock_insert: MagicMock, mock_xcom: MagicMock, mock_var: MagicMock):
    ...
    @patch("tasks.read.read_tasks.ReadTasks._get_file_names")
    @patch("tasks.read.read_tasks.ReadTasks._get_last_file_and_row")
    def test_read_image_no_files(self, mock_get_last_seen: MagicMock, mock_file_names: MagicMock, mock_files_to_df: MagicMock, mock_insert: MagicMock, mock_xcom: MagicMock):
        last_seen_file, last_seen_row = "someFile", 4
        files_to_read = []
        variable_key = "image_file_directory"
        data = pd.DataFrame(data={"ullid":[0, 1, 2, 3], "values": ["value1", "value2", "value3", "value4"]})
    ...
    @patch("tasks.read.read_tasks.ReadTasks._get_file_names")
    @patch("tasks.read.read_tasks.ReadTasks._get_last_file_and_row")
    def test_read_image_no_new_data(self, mock_get_last_seen: MagicMock, mock_file_names: MagicMock, mock_files_to_df: MagicMock,
